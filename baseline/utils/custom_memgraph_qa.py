#  Code adapted from langchain_community/chains/graph_qa/memgraph.py
...
Example:
# Can you describe the oversight responsibilities of the 
# Governing Body in ensuring that a Listed Fund's Reporting 
# Entity maintains the necessary systems and controls 
# for compliance?

MATCH (f {id: "Governing Body"})-[r]->(m {id: "Reporting Entity"})
RETURN f,r,m

MATCH (f {id: "Reporting Entity"})-[r]->(m)
RETURN f,r,m

MATCH (f)-[r]-(g {id: "Listed Fund"})
RETURN f,r,g

"""

# Takes as input user query and search for the schema components
PROMPT_EXTRACT_ENTITIES = """
Task:Generate Cypher statement to query a graph database to extract types and properties from the schema.
Instructions:
- Extract main search entities from the question that you will search for in the knowledge graph.
...
Form a list of entities separated by commas.
Example:
...
def get_function_response(
    ...
    """
    Takes as input user query and search for the schema components
    and returns a list of entities separated by commas.

    :param question: User query
    :param context: List of entities separated by commas
    :return: List of entities separated by commas
    """
...
class CustomMemgraphQAChain(Chain):
